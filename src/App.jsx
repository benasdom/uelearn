...
import {
  SmileOutlined, TeamOutlined, MoneyCollectOutlined, LogoutOutlined,
...
function App() {
  ...
  const suggestionRef = useRef(null)  // ref instead of document.querySelector
  ...
  }, []); // run once — don't put `shows` here, it causes a loop

  // ── listen for credit updates from the Payment page (cross-tab) ──
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (err instanceof AuthError) {
          // Token is gone or expired — show login modal, no reload
          setshows(true);
        }
        // Non-auth errors (network down etc.) are silent — cached localStorage
        // data is already applied and the user can still browse
      } finally {
  ...
  // ── fetch question bank — only once, not on every `find` change ──
  useEffect(() => {
    fetch(PASCO_API_URL)
      .then((res) => {
  ...
  // ── shared SearchList props ──
  const searchListProps = {
    setdataerror, setcourseName, setcredits,
    setpdflink, pdflink,
    setsearching, find,
    NetworkError, payload,
    bar, setfind,
    setactualDlink, actualDlink,
    dataerror, credits, courseName,
        ...
        <div className="footer">
          ...
          <div className="foot1">
            <div className="twwo" title="Make a suggestion">
              ...
              <div className="input">
                {/* ref instead of document.querySelector */}
                <input
                  ref={suggestionRef}
                  type="text"
                  className="find"
